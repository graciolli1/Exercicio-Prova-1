...
namespace Exercicio_Prova_1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string nome;
            double n1, n2, media, tra;
            Console.WriteLine("Digite seu nome: ");
            nome = Console.ReadLine();
            Console.WriteLine("Digite a nota do nota 1: ");
            n1 = Convert.ToDouble(Console.ReadLine());
            Console.WriteLine("Digite a nota 2: ");
            n2 = Convert.ToDouble(Console.ReadLine());
            Console.WriteLine("Digite a nota do trabalho: ");
            tra = Convert.ToDouble(Console.ReadLine());
            media = (n1 + n2 + tra) / 3;
            if (media >= 8.5)
            {
                Console.WriteLine(String.Format("A média do {0} é {1}. Conceito final é A.", nome, media.ToString("f")));
            }
            if (media > 7 && media < 8.4)
            {
                Console.WriteLine(String.Format("A média do {0} é {1}. Conceito final é B.", nome, media.ToString("f")));
            }
            if (media > 6 && media < 6.9)
            {
                Console.WriteLine(String.Format("A média do {0} é {1}. Conceito final é C.", nome, media.ToString("f")));
            }
            if (media > 0.1 && media < 5.9)
            {
                Console.WriteLine(String.Format("A média do {0} é {1}. Conceito final é D.", nome, media.ToString("f")));
            }
            if (media == 0)
            {
                Console.WriteLine(String.Format("A média do {0} é {1}. Conceito final é E.", nome, media.ToString("f")));
            }
            Console.ReadKey();
        }
    }
